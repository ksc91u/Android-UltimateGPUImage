...
//        filter.onOutputSizeChanged(width, height);
        filter.onOutputSizeChanged(displayWidth, displayHeight);
        int textureId = LitOpenGlUtils.loadTexture(bitmap, LitOpenGlUtils.NO_TEXTURE, true);
        if(rotate){
            FloatBuffer gLCubeBuffer = ByteBuffer.allocateDirect(LitTextureRotationUtil.CUBE.length * 4)
                    .order(ByteOrder.nativeOrder())
                    .asFloatBuffer();
            gLCubeBuffer.put(LitTextureRotationUtil.CUBE).position(0);
			...
			FloatBuffer gLTextureBuffer = ByteBuffer.allocateDirect(TextureRotationUtil.TEXTURE_NO_ROTATION.length * 4)
					.order(ByteOrder.nativeOrder())
					.asFloatBuffer();
			gLTextureBuffer.put(TextureRotationUtil.getRotation(Rotation.NORMAL, true, false)).position(0);
			filter.onDraw(textureId, gLCubeBuffer, gLTextureBuffer);
        }
